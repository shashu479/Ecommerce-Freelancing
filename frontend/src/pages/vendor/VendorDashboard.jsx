      ...
      >
        ...
        <nav className="p-4 space-y-1 h-[calc(100vh-280px)] overflow-y-auto scrollbar-thin">
          {menuItems.map((item) => (
            ...
            >
              <item.icon className={`w-5 h-5 transition-colors ${activeTab === item.id ? "text-accent" : "text-secondary group-hover:text-accent"}`} />
              <span className={`font-medium text-sm tracking-wide ${activeTab === item.id ? "font-bold" : ""}`}>{item.label}</span>
            </button>
          ))}
        </nav>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-secondary/5 border border-secondary/10 rounded-sm p-8 text-center backdrop-blur-sm">
        <h2 className="font-heading text-3xl font-bold text-primary mb-2">Need help? We're here for you.</h2>
        <p className="text-text-secondary max-w-lg mx-auto font-light">
          Our support team is available Mon-Fri to assist you with any issues related to your shop, orders, or account.
        </p>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-surface rounded-sm p-6 shadow-md border border-secondary/10 space-y-4 hover:border-accent/30 transition-colors">
            <div className="flex items-center gap-3 text-primary">
              ...
              <h3 className="font-heading font-semibold text-lg">Email Support</h3>
            </div>
            <div>
              <p className="font-medium text-primary">info@sirabaorganic.com</p>
              <p className="text-sm text-text-secondary font-light">Response time: 24 hours</p>
            </div>
          </div>
          ...
          <div className="bg-surface rounded-sm p-6 shadow-md border border-secondary/10 space-y-4 hover:border-accent/30 transition-colors">
            <div className="flex items-center gap-3 text-primary">
              ...
              <h3 className="font-heading font-semibold text-lg">Phone Support</h3>
            </div>
            <div>
              <p className="font-medium text-primary">+91 98765 43210</p>
              <p className="text-sm text-text-secondary font-light">Mon-Fri, 9am - 6pm IST</p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-surface rounded-sm p-8 shadow-md border border-secondary/10">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Subject</label>
                <select
                  value={subject}
              ...
              <div>
                <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">How can we help?</label>
                <textarea
                  value={message}
  ...
  <div className="bg-surface rounded-sm p-6 shadow-md border border-secondary/10 hover:border-accent/30 transition-colors group">
    <div className="flex items-start justify-between">
      <div>
        ...
        <p className="text-3xl font-heading font-medium mt-1 text-primary">{value}</p>
        {subtitle && (
          <p className="text-xs text-text-secondary mt-1 font-light italic">{subtitle}</p>
        )}
        {trend && (
          <div
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-primary to-primary/90 rounded-sm p-6 text-surface flex items-center justify-between shadow-xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-5 relative z-10">
          ...
          <div>
            <p className="font-heading font-medium text-xl">Your Commission Rate</p>
            <p className="text-surface/80 text-sm font-light mt-1">
              Platform fee on each sale (
              <span className="font-medium text-accent">{vendor?.subscription?.plan || "Starter"}</span> plan)
            </p>
          </div>
        </div>
      ...
      {/* Orders by Status */}
      <div className="bg-surface rounded-sm p-8 shadow-md border border-secondary/10">
        <h3 className="font-heading font-medium text-xl mb-6 text-primary">Order Status Overview</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
          {[
            ...
            <div key={status} className="text-center p-4 bg-background/50 rounded-sm hover:bg-secondary/5 transition-colors border border-transparent hover:border-secondary/10">
              <p className="text-2xl font-bold font-heading text-primary mb-1">
                {ordersByStatus[status] || 0}
              </p>
              <p className="text-xs text-text-secondary capitalize tracking-wide">{status}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-surface rounded-sm p-6 shadow-md border border-secondary/10">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-heading font-medium text-lg text-primary">Recent Orders</h3>
            <Link to="#" className="text-accent text-xs font-bold uppercase tracking-wider hover:text-primary transition-colors">
              View All
            </Link>
          </div>
              ...
              {recentOrders.slice(0, 5).map((order) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="font-medium text-primary">₹{order.subtotal}</p>
                    <span
          ...
          {lowStockItems.length > 0 ? (
            <div className="space-y-3">
              {lowStockItems.slice(0, 5).map((item) => (
                ...
                >
                  <div>
                    <p className="font-medium text-sm text-primary">{item.sku}</p>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-surface rounded-sm p-8 max-w-md w-full shadow-2xl border border-secondary/10">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Status</label>
                <select
      ...
      <div className="bg-surface rounded-sm shadow-lg border border-secondary/10 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-secondary/5">
              {returns.map((ret) => (
                <tr key={ret._id} className="hover:bg-secondary/5 transition-colors">
                  <td className="px-6 py-5 text-sm font-medium text-primary">
                    #{ret._id.slice(-8)}
                  </td>
                  <td className="px-6 py-5 text-sm text-text-secondary font-light">
      ...
      {selectedReturn && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-surface rounded-sm p-8 max-w-md w-full shadow-2xl border border-secondary/10">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Current Status</label>
                <p className="text-lg font-medium text-primary">{selectedReturn.status}</p>
              </div>
              <div>
                <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Update Status</label>
                <select
                  value={updateStatus}
              ...
              {selectedReturn.customerComment && (
                <div>
                  <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Customer Comment</label>
            ...
            <div className="flex gap-4 mt-8 pt-4 border-t border-secondary/5">
              ...
              {['Approved'].includes(selectedReturn.status) && selectedReturn.returnStatus !== 'Completed' && selectedReturn.returnStatus !== 'Refunded' && (
                <button
                  onClick={async () => {
                    if (!window.confirm("Are you sure you want to issue a refund? This will deduct from your wallet/payouts.")) return;
                    setLoading(true);
                    try {
                      await client.post(`/vendors/orders/${selectedReturn._id}/refund`);
    ...
    <div className="space-y-6">
        ...
        <div className="bg-yellow-50/50 border border-yellow-200 rounded-sm p-5 flex items-start gap-4 backdrop-blur-sm">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-800/80 font-light mt-1 max-w-2xl">
              ...
              account is approved. Our team reviews accounts typically within 24-48 hours.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-secondary/5 border border-secondary/10 rounded-sm p-6 flex gap-4 backdrop-blur-sm">
        ...
        <p className="text-sm text-text-secondary font-light leading-relaxed max-w-4xl">
          <strong className="text-primary font-medium block mb-1">How it works:</strong> Products you add will be reviewed by
          our team before appearing in the shop. Once approved, customers can
          purchase them. You'll receive orders and payouts through the platform.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {vendorProducts.map((product) => (
          ...
          >
            ...
            {/* Product Info */}
            <div className="p-6">
              <h3 className="font-heading font-medium text-lg text-primary truncate mb-1">{product.name}</h3>
              ...
              <div className="flex items-end justify-between mb-4 pb-4 border-b border-secondary/5">
                <div>
                  <p className="font-bold text-xl text-primary font-heading">₹{product.price}</p>
                  {product.costPrice && (
      ...
      {vendorProducts.length === 0 && (
        <div className="bg-surface rounded-sm p-16 text-center border border-secondary/10">          <Package className="w-12 h-12 text-gray-400 mx-auto mb-4" />
          ...
          <div className="bg-surface rounded-sm p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-secondary/10">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Tag</label>
                  <input
                    type="text"
                    value={formData.tag}
                    onChange={(e) =>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {formData.features.map((feat, i) => (
                    ...
                    >
                      {feat}
                      <button type="button" onClick={() => removeFeature(i)} className="text-text-secondary hover:text-red-500">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  ))}
...
const ComplianceContent = ({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const result = await addComplianceDoc(formData);
    if (result.success) {
      setShowAddModal(false);
      setFormData({
        name: "",
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-6 border-b border-secondary/10 pb-6">
        <div>
          <h2 className="text-2xl font-heading text-primary">Compliance Documents</h2>
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-heading font-medium text-lg text-primary">{doc.name}</p>
            ...
            {doc.expiryDate && (
              <p className="text-xs text-text-secondary mt-4 font-light border-t border-secondary/5 pt-3">
                Expires: <span className="font-medium text-primary">{new Date(doc.expiryDate).toLocaleDateString()}</span>
              </p>
            )}
            ...
            <div className="flex gap-4 mt-6 pt-4 border-t border-secondary/5">
              <a
                href={doc.fileUrl}
                target="_blank"
      ...
      {compliance.length === 0 && (
        <div className="bg-surface rounded-sm p-16 text-center shadow-sm border border-secondary/10">
          ...
          <p className="text-text-secondary font-light">No documents uploaded yet</p>
          <button
            onClick={() => setShowAddModal(true)}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-text-secondary mt-1">
                  Upload to cloud storage and paste the URL
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">
...
const SettingsContent = ({ vendor, updateProfile, getProfile }) => {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <h2 className="text-2xl font-heading font-medium text-primary">Account Settings</h2>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Phone</label>
            <input
              type="tel"
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Website</label>
            <input
              type="url"
        ...
        <h3 className="font-heading font-medium text-lg text-primary">Legal Information</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">GST Number</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">PAN Number</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">FSSAI Number</label>
            <input
              type="text"
        ...
        <h3 className="font-heading font-medium text-lg text-primary">Address</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">City</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">State</label>
            <input
              type="text"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-surface rounded-sm p-6 shadow-md border border-secondary/10 hover:border-green-300 transition-colors">
          ...
          <p className="text-sm text-text-secondary mt-1 font-light">Available Balance</p>
        </div>
        ...
        <div className="bg-surface rounded-sm p-6 shadow-md border border-secondary/10 hover:border-yellow-300 transition-colors">
          ...
          <p className="text-sm text-text-secondary mt-1 font-light">Pending Earnings</p>
        </div>
        ...
        <div className="bg-surface rounded-sm p-6 shadow-md border border-secondary/10 hover:border-primary/30 transition-colors">
          ...
          <p className="text-sm text-text-secondary mt-1 font-light">Total Earnings</p>
        </div>
        ...
        <div className="bg-surface rounded-sm p-6 shadow-md border border-secondary/10 hover:border-orange-300 transition-colors">
          ...
          <p className="text-sm text-text-secondary mt-1 font-light">Commission Paid</p>
        </div>
      </div>
      ...
      <div className="bg-secondary/5 border border-secondary/10 rounded-sm p-6 flex items-start gap-4">
        ...
        <div>
          <p className="font-heading font-medium text-lg text-primary mb-1">When do I get paid?</p>
          <p className="text-sm text-text-secondary font-light leading-relaxed">
            Your wallet balance is credited when orders are marked as{" "}
            <strong className="text-primary font-medium">"Delivered"</strong>. Orders in pending, confirmed, or
            shipped status will appear in your dashboard stats but won't be
            added to your wallet until delivery is confirmed.
          </p>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-sm p-8 shadow-md border border-secondary/10">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-heading font-medium text-xl text-primary mb-1">Request Payout</h3>
            <p className="text-sm text-text-secondary font-light">
              Minimum payout: <span className="font-medium text-primary">₹500</span> | Payouts processed within 3-5 business days
            </p>
          </div>
          <button
      ...
      <div className="bg-surface rounded-sm shadow-lg border border-secondary/10 overflow-hidden">
        <div className="p-6 border-b border-secondary/10 bg-background/30">
          <h3 className="font-heading font-medium text-lg text-primary">Transaction History</h3>
        </div>
              ...
              >
                ...
                <div className="text-right">
                  {(() => {
                    ...
                    const isCredit = ["order_earning", "credit", "adjustment"].includes(txn.type);
                    const isDebit = ["payout", "commission", "debit", "refund_debit", "pending_cancelled", "admin_refund_debit", "admin_pending_cancelled"].includes(txn.type);
                    ...
                    return (
                      <p className={`font-bold font-heading text-lg ${colorClass}`}>
                        {sign}₹{Math.abs(txn.amount).toLocaleString()}
                      </p>
                    );
                  })()}
                  <p className="text-xs text-text-secondary font-light mt-1">
                    Balance: <span className="font-medium text-primary">₹{(txn.balanceAfter || 0).toLocaleString()}</span>
                  </p>
                </div>
              </div>
            ))
      ...
      {showPayoutModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-surface rounded-sm p-8 max-w-md w-full shadow-2xl border border-secondary/10">
            <h3 className="text-2xl font-heading font-medium mb-6 text-primary border-b border-secondary/10 pb-4">Request Payout</h3>
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">
                  Amount (₹)
                ...
                <p className="text-xs text-text-secondary mt-2 font-light">
                  Available: <span className="font-medium text-green-600">₹{(walletData?.balance || 0).toLocaleString()}</span>
                </p>
              </div>
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      <div className="flex items-center justify-between border-b border-secondary/10 pb-6">
        <div>
          <h2 className="text-2xl font-heading text-primary font-medium">Shop Settings</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-surface rounded-sm p-8 shadow-md border border-secondary/10 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Logo URL</label>
              <input
                type="url"
        ...
        <div className="bg-surface rounded-sm p-8 shadow-md border border-secondary/10 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Facebook</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">Twitter</label>
              <input
                type="text"
        ...
        <div className="bg-surface rounded-sm p-8 shadow-md border border-secondary/10">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-heading font-medium text-lg text-primary">Publish Shop</h3>
...
const SubscriptionContent = ({
  ...
  return (
    <div className="space-y-6">
        ...
        {plans &&
          Object.entries(plans).map(([key, plan]) => {
            ...
            return (
              <div
                key={key}
                className={`relative bg-surface rounded-sm p-8 shadow-lg border transition-all duration-300 hover:scale-105 ${isCurrentPlan ? "border-accent shadow-xl ring-1 ring-accent/20" : "border-secondary/10 hover:border-accent/50"
                  }`}
              >
                {plan.badge && (
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h3 className="font-heading font-medium text-xl text-primary">{plan.name}</h3>
                </div>
                ...
                <p className="text-sm font-medium text-text-secondary mb-6 flex items-center gap-2">
                  <span className="text-accent font-bold text-lg">{plan.commissionRate}%</span> platform commission
                </p>

                <ul className="space-y-3 mb-8">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm font-light text-text-primary">
      ...
      {/* Features Comparison */}
      <div className="bg-surface rounded-sm p-8 shadow-md border border-secondary/10">
        <h3 className="text-xl font-heading font-medium text-primary mb-6 border-b border-secondary/10 pb-4">Plan Benefits</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-secondary/10">
                <th className="text-left py-4 px-6 font-heading font-medium text-primary">Feature</th>
                <th className="text-center py-4 px-6 font-heading font-medium text-primary">Starter</th>
                <th className="text-center py-4 px-6 font-heading font-medium text-primary">Professional</th>
                <th className="text-center py-4 px-6 font-heading font-medium text-primary">Enterprise</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-secondary/5">
              <tr>
                ...
                <td className="text-center py-4 px-6 text-sm font-medium">Unlimited</td>
              </tr>
              <tr>
                <td className="py-4 px-6 text-sm font-light">Commission Rate</td>
                <td className="text-center py-4 px-6 text-sm font-medium">15%</td>
                <td className="text-center py-4 px-6 text-sm font-medium">10%</td>
                <td className="text-center py-4 px-6 text-sm font-medium">5%</td>
              </tr>
              <tr>
                <td className="py-4 px-6 text-sm font-light">Priority Support</td>
              ...
              <tr>
                <td className="py-4 px-6 text-sm font-light">Analytics Dashboard</td>
                <td className="text-center py-4 px-6 text-sm">Basic</td>
                <td className="text-center py-4 px-6 text-sm">Advanced</td>
                <td className="text-center py-4 px-6 text-sm">Advanced + Reports</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-surface rounded-sm max-w-md w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-secondary/10">
            ...
            <div className="p-6 bg-background/50 border-b border-secondary/10">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-heading font-medium text-lg text-primary">{selectedPlan.name} Plan</p>
                  <p className="text-sm text-text-secondary font-light">
              ...
              <div className="mt-4 p-3 bg-secondary/10 rounded-sm border border-secondary/20">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-primary font-medium">
                    ...
                    <span className="text-green-600 font-bold">{selectedPlan.commissionRate}%</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="p-4 bg-orange-50 rounded-sm border border-orange-100 text-sm">
                <p className="text-orange-800 font-light">
                  <strong className="font-medium">Demo Mode:</strong> Use any card details for testing.
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-text-secondary uppercase tracking-wide">CVV</label>
                  <input
                    type="password"
              ...
              <div className="flex items-center justify-center gap-2 text-xs text-text-secondary py-2 border-t border-secondary/5 pt-4">
                ...
                <span className="font-light tracking-wide uppercase">256-bit SSL Encrypted Payment</span>
              </div>
            </div>
...
const ReviewsContent = ({ vendorProducts }) => {
  ...
  const [replyModal, setReplyModal] = useState({ show: false, reviewId: null, productId: null, currentReply: "" });
  ...
  const handleReplySubmit = async (e) => {
    ...
    try {
      await client.put(`/products/${replyModal.productId}/reviews/${replyModal.reviewId}/reply`, {
        reply: replyText
      });

      // Update reviews state optimistically
      setReviews(prev => prev.map(review =>
        review._id === replyModal.reviewId && review.productId === replyModal.productId
          ? { ...review, vendorReply: replyText, vendorReplyDate: new Date() }
          : review
      ));

      alert("Reply submitted");
      setReplyModal({ show: false, reviewId: null, productId: null, currentReply: "" });
      setReplyText("");
    } catch (error) {
  ...
  return (
    <div className="space-y-8">
      <h2 className="text-2xl font-heading font-medium text-primary border-b border-secondary/10 pb-6">Product Reviews</h2>

      <div className="space-y-6">
        {reviews.length > 0 ? (
          reviews.map(review => (
            <div key={review._id} className="bg-surface p-8 rounded-sm border border-secondary/10 shadow-md hover:shadow-lg transition-shadow duration-300">
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-start gap-6">
                  <img src={review.productImage} alt="" className="w-16 h-16 object-cover rounded-sm border border-secondary/20 shadow-sm" />
                  <div>
                    <p className="font-heading font-medium text-lg text-primary">{review.productName}</p>
                    <div className="flex items-center gap-3 mt-1.5">
                      ...
                      <span className="text-xs font-bold bg-accent/10 text-accent px-2 py-0.5 rounded-sm">{review.rating}/5</span>
                    </div>
                  </div>
                </div>
                <span className="text-xs text-text-secondary font-light bg-secondary/5 px-3 py-1 rounded-full">{new Date(review.createdAt).toLocaleDateString()}</span>
              </div>

              <div className="mb-6 pl-22">
                <p className="font-medium text-primary mb-2 flex items-center gap-2">
                  {review.name}
                  <span className="text-[10px] uppercase tracking-wider text-text-secondary font-light border border-secondary/20 px-1.5 rounded-sm">Verified Buyer</span>
                </p>
                <p className="text-text-secondary font-light leading-relaxed bg-background/50 p-4 rounded-sm border border-secondary/5 italic">"{review.comment}"</p>
              </div>
              ...
              {review.vendorReply && (
                <div className="bg-secondary/5 p-6 rounded-sm border-l-4 border-accent mb-6 ml-6">
                  ...
                  <p className="text-sm text-text-secondary font-light">{review.vendorReply}</p>
                </div>
              )}
          ...
          <div className="text-center py-24 bg-surface rounded-sm border border-secondary/10 shadow-sm">
            ...
            <p className="text-text-secondary font-light text-lg">No reviews found yet.</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-surface rounded-sm p-8 max-w-lg w-full shadow-2xl border border-secondary/10">
            ...
            <form onSubmit={handleReplySubmit}>
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button
                  type="button"
                  onClick={() => setReplyModal({ show: false, reviewId: null, productId: null })}
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-140px)] flex flex-col">
      <div className="flex items-center justify-between border-b border-secondary/10 pb-4">
        <div>
          <h2 className="text-2xl font-heading font-medium text-primary">Messages</h2>
          <p className="text-text-secondary font-light mt-1">Direct communication with Support</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-background/30">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-text-secondary opacity-60">
              ...
              <p className="font-heading font-medium text-lg text-primary">No messages yet</p>
              <p className="font-light text-sm mt-1">Start a conversation with support team.</p>
            </div>
          ) : (
            messages.map((msg) => (
              <div
              ...
              >
                ...
                >
                  <p className={`text-sm leading-relaxed ${msg.sender === "vendor" ? "font-light" : "font-normal"}`}>{msg.message}</p>
                  <p
      ...
      <div className="lg:ml-64">
          ...
          <div className="bg-accent text-primary px-8 py-4 sticky top-16 z-20 shadow-md backdrop-blur-md bg-accent/90">
            <div className="flex items-center justify-center gap-3">
              ...
              <span className="font-heading font-medium tracking-wide">
                Your account is <span className="font-bold underline decoration-wavy underline-offset-4">{vendor.status?.replace("_", " ")}</span> - Some
                features are restricted until approval
              </span>
            </div>
          </div>
        )}
        ...
        <header className="bg-surface/80 backdrop-blur-md shadow-sm border-b border-secondary/10 sticky top-0 z-30 transition-all duration-300">
          <div className="flex items-center justify-between px-8 py-5">
            ...
            <div className="flex items-center gap-6">
              <NotificationDropdown notifications={notifications} onMarkAsRead={handleMarkAllRead} />
        ...
        <main className="p-8 max-w-[1600px] mx-auto animate-fadeIn">
          ...
          {activeTab === "messages" && (
            <MessagesContent vendor={vendor} />
          )}
          {activeTab === "reviews" && (
            <ReviewsContent vendorProducts={vendorProducts} />
          )}
          {activeTab === "support" && (
            <SupportContent vendor={vendor} />
          )}
          {activeTab === "settings" && (
            <SettingsContent
              vendor={vendor}
